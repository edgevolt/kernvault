...
- **Read Aloud** — plays the article as speech, sentence by sentence, highlighting each sentence as it is spoken. Voice and speed are adjustable. Speech is generated by a small neural text-to-speech model that runs **entirely on your own server** — no article text or audio is ever sent to a third party. Turn it on per-browser with the **Read aloud** switch in the reader's display (Aa) menu; a play control then appears in the toolbar. It's off until you flip that switch, so it never uses CPU unprompted. Operators can hard-disable it server-wide on low-power hosts (see [Configuration](#configuration)).
...
A small neural TTS model (Kokoro-82M) runs **in-process on the host CPU** and streams audio to
the browser — nothing is sent to any third party. The Docker image **ships with the model baked
in**, so the feature is **available by default** with no setup. It follows a
capability-vs-preference split:

- **Availability (operator):** available by default when the model is present. Set
  `TTS_ENABLED=false` to hard-disable it server-wide (e.g. on a low-power host).
- **Activation (user):** even when available, nothing runs until a user turns on the **Read
  aloud** switch in the reader's display (Aa) menu. That preference is per-browser
  (`localStorage`) and defaults off, so the host never spends CPU on speech unprompted.
...
is playing; on a low-power host, simply leave `TTS_ENABLED=false` (the button won't appear).
Synthesized audio is cached under `data/tts-cache/` (regenerable, safe to delete). Maintainers
...
- Read-aloud (text-to-speech), when enabled, runs entirely on your own server — article text and generated audio never leave the machine.
